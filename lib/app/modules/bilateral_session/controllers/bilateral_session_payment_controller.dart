...
import '../models/add_bilateral_session_coupon.dart';
import '../models/get_bilateral_session_details_model.dart';
import '../providers/add_bilateral_session_coupon_provider.dart';
import 'bilateral_session_details_controller.dart';

class BilateralSessionPaymentController extends GetxController {
  GetBilateralSessionDetailsModel bilateralData =
  ...
  final bilateralSCD = Get.put(BilateralSessionDetailsController());

  final GlobalKey<FormState> couponFormKey = GlobalKey<FormState>();

  AddBilateralSessionCoupon couponData = AddBilateralSessionCoupon();

  set setCoupon(AddBilateralSessionCoupon data) {
    couponData = data;
    update();
  }

  AddBilateralSessionCoupon get getCoupon => couponData;
  ...
  var couponProvider = AddBilateralSessionCouponProvider();

  applyCoupon() async {
    try {
      var data = await couponProvider.addCoupon(
        couponCode: couponCode,
        appointmentId: bilateralSCD.getSBSD.data!.appointmentId!,
      );
      setCoupon = data;
